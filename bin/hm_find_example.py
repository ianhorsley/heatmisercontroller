...
for current_controller in HMN.controllers:
    print("\r\nGetting all data control %2d in %s *****************************" %(
            current_controller.address, current_controller.long_name))

    try:
        current_controller.read_all()
    except HeatmiserResponseError as err:
        print("C%d in %s Failed to Read due to %s" %(current_controller.address,
        ...
        disptext = "C%d Air Temp is %.1f from type %.f and Target set to %d   Boiler Demand %d"%(
                    ...
                    current_controller.read_air_sensor_type(),
                    current_controller.setroomtemp, current_controller.heatingdemand)
